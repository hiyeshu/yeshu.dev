...
<config>
.github/CLAUDE.md - L2 GitHub 配置地图，声明 GitHub 不承担生产发布
...
package.json - npm 依赖与脚本，声明 Astro、React island、Three.js、TailwindCSS v4、shadcn、UI 增强库和锁定版 Wrangler
...
wrangler.jsonc - Cloudflare Worker 部署配置，固定账号、`yeshu-dev`、Astro 构建命令、dist assets、trailing slash、兼容 flag 和既有 SESSION KV
...
## 变更日志
- 2026-06-07: 收敛发布真相源到 Cloudflare Worker Builds，删除 GitHub Actions 部署入口；`wrangler.jsonc` 显式写入 account_id 与 `build.command`，并把 Wrangler 锁进 devDependency，避免 Dashboard 构建命令缺失或临时下载新版导致部署漂移。
